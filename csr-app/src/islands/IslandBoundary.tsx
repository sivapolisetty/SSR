...
export const IslandBoundary: React.FC<IslandBoundaryProps> = ({
  ...
  const fetchSSRHtml = useCallback(async (componentName: string, componentProps: Record<string, any>) => {
    try {
  ...
  const loadRemoteComponent = useCallback(async (remoteName: string, componentName: string) => {
    try {
      setState(prev => ({ ...prev, isLoading: true, hasError: false }));
  ...
  }, [name, strategy, priority, remoteModule, remoteName, errorBoundary, onHydrated, props, children, dependencies]);
